...
AUTH0_DOMAIN = 'dev-qucb5vmv.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'drink'
...
def get_token_auth_header():
    ...
    """
    fetch_auth = request.headers.get('Authorization', None)
    if not fetch_auth:
        raise AuthError({
    ...
    separate_auth = fetch_auth.split()
    if separate_auth[0].lower() != 'bearer':
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Authorization header must start with "Bearer".'
        }, 401)

    elif len(separate_auth) == 1:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Token not found.'
        }, 401)

    elif len(separate_auth) > 2:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Authorization header must contain only bearer and token'
        }, 401)

    token = separate_auth[1]
    return token
